...
const ElmProfile = () => {
      ...
      setStock((prev) =>
        prev.map((item) =>
          item.id === selectedItem.id
            ? {
                ...item,
                jbcareId: formData?.jbcareId,
                elmId: formData?.elmId,
                connectionNo: formData?.connectionNo,
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = stock.filter((item) => {
      const matchesSearch =
        item?.jbcareId.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item?.elmId.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="bg-white">
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">ELM Management</h2>
            <p className="text-gray-600 mt-1">Manage ELM Customers</p>
          </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {paginatedData?.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50">
                <td className="px-6 py-4">{item?.jbcareId}</td>
                <td className="px-6 py-4">{item?.elmId}</td>
                <td className="px-6 py-4">{item?.connectionNo}</td>
                <td className="px-6 py-4">{item?.meterNo}</td>
                <td className="px-6 py-4">{item?.openingReading}</td>
